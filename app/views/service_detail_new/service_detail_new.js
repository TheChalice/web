/**
 * Created by zhangning on 2017/3/24.
 */
'use strict';
angular.module('console.service.detail', [
    ...
    {
        files: [
            'views/service_detail_new/service_detail_new.css',
    ...
    function ($sce, ansi_ups, $http, $state, $rootScope, $scope, $log, $stateParams, DeploymentConfig, ReplicationController, Route, BackingServiceInstance, ImageStream, ImageStreamTag, Toast, Pod, Event, Sort, Confirm, Ws, LogModal, Secret, ImageSelect, Service, BackingServiceInstanceBd, ImageService, serviceaccounts, ChooseSecret, $base64, secretskey,Metrics,MetricsService,ContainerModal_new) {
        ...
        var setChart = function (i) {
            var cpuData = $scope.cpuData;
            var memData = $scope.memData;
            angular.forEach(cpuData,function(item,i){
                if(item===null){
                    cpuData[i]=0;
                }
            })
            angular.forEach(memData,function(item,i){
                if(item===null){
                    memData[i]=0;
                }
            })
            if(arguments.length!==0){
                if(arguments[0] === 'cpu'){
                    memData =null;
                    cpuData = $scope.cpuData;
                    $scope.isGray['neicun'] = false;
                    $scope.isGray['cpu'] = true;

                }else{
                    cpuData =null;
                    memData = $scope.memData;
                    $scope.isGray['cpu'] = false;
                    $scope.isGray['neicun'] = true;
                }
            }
            return {
                options: {
        ...
        $scope.changeChart = function(i){
            $scope.chartConfig = setChart(i);
        ...
        var getMetrics = function (pod, container) {
            ...
            Metrics.mem.query({gauges: gauges, buckets: 61, start: st}, function (res) {
                ...
                Metrics.cpu.query({counters: counters, buckets: 61, start: st}, function (res) {
                    ...
                    $scope.cpuData = prepareData('CPU', []);
                    $scope.grid.cpu = false;

                });
            }, function (res) {
                // $log.info("metrics mem err", res);
                $scope.memData = prepareData('内存', []);
      ...
      var checkMetrics = function(name,idx) {
        ...
        $scope.$watch('dc.status.replicas', function (n, o) {
            ...
            if ($scope.dc) {
                ...
                }else if (n == $scope.dc.spec.replicas) {
        ...
        var loadRoutes = function (dc,sever) {
            ...
            Route.get({namespace: $rootScope.namespace,name:dc.metadata.name,region:$rootScope.region}, function (res) {
                $log.info("routes", res);
                angular.forEach($scope.service.spec.ports, function (port,i) {
                    angular.forEach($scope.dc.spec.template.spec.containers, function (con,k) {
                        if (con.name === port.name.split('-')[1]) {
                            con.port=port
                        }
                    })
                })
                //$scope.service.spec.ports
                $scope.routeconfed.checkcon=res.spec.port.targetPort.split('-')[1];
                if (!$scope.routeconf) {
